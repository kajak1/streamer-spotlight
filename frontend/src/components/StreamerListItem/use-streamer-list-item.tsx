...
function useStreamerListItem({ id, name }: Props) {
  const {
    ...
    isLoading,
    error,
  } = useSWR([SWR_KEYS.STREAMERS, id], () => streamersService.getVoteCount(id));
  ...
  useEffect(() => {
    const voteHandler: ServerToClientEvents["VOTE"] = (updated) => {
      ...
      mutate(
        ...
        votes
          ? { ...votes, upvotes: updated.upvotes, downvotes: updated.downvotes }
          : votes,
      );
  ...
  function handleVote(
    voteType: VoteTypeBody["voteType"],
    operation: VoteTypeBody["operation"],
  ) {

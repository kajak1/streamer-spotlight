...
class UsersService extends Service {
  ...
  getData = async () => {
    const response = await this.api.get<User>(`${this.USERS_ENDPOINT}/me`);

    return response.data;
  };

  getVotes = async () => {
    const response = await this.api.get<GetVotesResponse>(
  ...
  getVotesOnStreamer = async (streamerId: string) => {
    const response = await this.api.get<GetVotesOnStreamerResponse>(

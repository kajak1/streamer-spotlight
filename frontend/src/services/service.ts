...
export abstract class Service {
	...
	private createRequestCookieInterceptor = (cookies: Partial<{ [key: string]: string }>, url: string): number => {
	...
	attachCookiesOnce_SSR =
		<T, R>(fn: (...args: T[]) => Promise<R>, ...args: T[]) =>
		async (...secondArgs: Parameters<typeof this.createRequestCookieInterceptor>): Promise<R> => {
